"""LLM-powered prompt complexity classifier.
...
judges how hard it would be to answer. This mirrors a real production
...
output that doesn't validate against `ClassificationResult`. The fallback
...
from backend.schemas.classification import ClassificationOutcome, ClassificationResult
...
def _build_classifier_prompt(user_prompt: str) -> str:
    ...
    return (
        "Classify the complexity of the prompt below for LLM routing purposes. "
        'Respond with JSON only: {"complexity": "Easy"|"Medium"|"Hard", '
        '"confidence": <0-1 float>, "reason": "<short reason, max 20 words>"}. '
...
class ClassifierService:
    ...
    def classify(self, prompt: str) -> ClassificationOutcome:
        """Classify a prompt, falling back to heuristics on any LLM failure."""
        ...
        try:
            ...
            result, input_tokens, output_tokens = self._classify_with_llm(prompt)
            ...
            result = classify_heuristically(prompt, self._settings)
            fallback_used = True
    ...
    def _classify_with_llm(self, prompt: str) -> tuple[ClassificationResult, int, int]:
